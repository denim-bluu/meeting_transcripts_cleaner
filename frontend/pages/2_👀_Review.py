"""
Review Page - Interface for reviewing cleaned transcripts with confidence scores.

This page shows processing results from the backend API including quality assessment,
confidence scores, and allows export in multiple formats.
"""
...
def main():
    ...
    st.markdown("Review your processed transcript with quality assessment and export options.")
    ...
    processing_stats = transcript.get("processing_stats", {})
    ...
    if cleaned_chunks and review_results:
        ...
        avg_quality = sum(r.get('quality_score', 0) for r in review_results if r) / len(review_results) if review_results else 0
        ...
        with tabs[0]:  # By Chunks
            ...
            if cleaned_chunks and review_results:
                ...
                for i, (chunk, clean_result, review) in enumerate(
                    ...
                    quality_icon, quality_text, quality_color = get_quality_status(quality_score)
                ...
                st.info("No detailed cleaning analysis available. Showing basic chunk information.")
                for i, chunk in enumerate(chunks):
                    entries = chunk.get("entries", [])
                    if not entries:
                        continue
