...
class SimpleTaskCache:
    ...
    async def get_task_for_idempotency_key(self, key: str) -> Optional[str]:
        """
    ...
    async def _cleanup_expired_tasks(self) -> Dict[str, int]:
        ...
        expired_keys = [
            key for key, expires_at in self._idempotency_expires.items()
            if expires_at <= now
        ]
...
_task_cache: Optional[SimpleTaskCache] = None
...
def get_task_cache() -> SimpleTaskCache:
    ...
    Returns:
        The global SimpleTaskCache instance
...
def initialize_cache(ttl_hours: int = 1, cleanup_interval_minutes: int = 10) -> None:
    ...
    Args:
        ttl_hours: Default TTL for tasks
