...
class IntelligenceProcessor:
    ...
    async def process_meeting(
        ...
        for i, chunk_text in enumerate(semantic_chunks):
            ...
            chunk_insights = await self._extraction.extract_insights(
                chunk_text, context
            )
            ...
            if progress_callback:
                progress_callback(
                    0.3 + 0.5 * (i + 1) / len(semantic_chunks),
                    f"Processed {i + 1}/{len(semantic_chunks)} chunks",
                )

        # Phase 3: Synthesize intelligence
        if progress_callback:
            progress_callback(0.9, "Synthesizing meeting intelligence...")
        intelligence = await self._synthesis.synthesize(insights)
