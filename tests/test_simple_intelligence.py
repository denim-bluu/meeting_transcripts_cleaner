...
class TestIntelligenceOrchestrator:
    ...
    async def test_process_meeting(self, sample_vtt_chunks):
        ...
        with patch.object(service.chunker, "create_chunks") as mock_chunk:
            ...
            with patch.object(
                service.extractor, "extract_all_insights", new_callable=AsyncMock
            ) as mock_extract:
                ...
                with patch.object(service, "_estimate_tokens") as mock_estimate:
                    mock_estimate.return_value = (
                        10000  # Small enough for direct synthesis
                    )

                    # Mock direct synthesis
                    with patch.object(
                        service.synthesizer,
                        "synthesize_intelligence_direct",
                    ...
                    assert (
                        result.action_items[0].description == "Review budget by Friday"
                    )
        ...
        with patch.object(service.chunker, "create_chunks") as mock_chunk:
            ...
            with patch.object(
                ...
                with patch.object(service, "_estimate_tokens") as mock_estimate:
                    ...
                    with patch.object(
                        service.synthesizer,
                        "synthesize_intelligence_hierarchical",
                        new_callable=AsyncMock,
                    ) as mock_synth:
                        mock_result = MeetingIntelligence(
                            ...
                            action_items=[
                                ActionItem(
                                    description="Follow up on long meeting outcomes",
                                    owner="Team",
                                    due_date="Friday",
                                )
                            ],
...
class TestIntelligenceModels:
    ...
    def test_action_item_validation(self):
        ...
        # Valid action item
        action = ActionItem(
            description="Complete the budget review", owner="John", due_date="Friday"
        )
        ...
        with pytest.raises(ValueError):
            ActionItem(description="Too short")  # type: ignore

        # Test optional fields
        action_minimal = ActionItem(description="Valid description here")  # type: ignore
