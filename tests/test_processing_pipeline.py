...
async def test_complete_pipeline():
    ...
    if cleaned_chunks and review_results:
        accepted_count = sum(1 for r in review_results if getattr(r, "accept", False))
        avg_confidence = sum(getattr(c, "confidence", 0) for c in cleaned_chunks) / len(
            cleaned_chunks
        )
        avg_quality = sum(getattr(r, "quality_score", 0) for r in review_results) / len(
            review_results
        )
