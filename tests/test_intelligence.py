...
class TestIntelligenceAgents:
    ...
    def test_summary_extractor_initialization(self):
        ...
        extractor = SummaryExtractor("test-key", "o3-mini")
        assert extractor.model_name == "o3-mini"
    ...
    def test_action_item_extractor_initialization(self):
        ...
        extractor = ActionItemExtractor("test-key", "o3-mini")
        assert extractor.model_name == "o3-mini"
        assert extractor.agent is not None
    ...
    def test_intelligence_synthesizer_initialization(self):
        ...
        synthesizer = IntelligenceSynthesizer("test-key", "o3-mini")
        assert synthesizer.model_name == "o3-mini"
        ...
        extractions = [
            {
                ...
                "actions": [
                    ActionItem(
                        description="Complete task 1", source_chunks=[1], confidence=0.8
                    )
                ],
            }
        ]
...
class TestIntelligenceService:
    ...
    def test_create_context_windows(self, sample_chunks):
        ...
        assert (
            "I'll handle the documentation" in windows[0]["full_context"]
        )  # Context after

        # Second window should have context before
        assert windows[1]["chunk_id"] == 1
        assert (
            "We need to complete the project" in windows[1]["full_context"]
        )  # Context before
    ...
    def test_determine_review_level(self):
        ...
        assert (
            service.determine_review_level(high_confidence_result) == ReviewLevel.NONE
        )
        ...
        assert (
            service.determine_review_level(medium_confidence_result)
            == ReviewLevel.LIGHT
        )
        ...
        assert (
            service.determine_review_level(low_confidence_result)
        ...
        assert (
            service.determine_review_level(critical_content_result)
            == ReviewLevel.DETAILED
        )
    ...
    async def test_extract_intelligence_basic_flow(self, sample_chunks):
        ...
        mock_action = ActionItem(
            description="Test action item", source_chunks=[1], confidence=0.8
        )
        ...
        with (
            patch.object(
                service.summary_extractor, "extract", new_callable=AsyncMock
            ) as mock_summary_extract,
            patch.object(
                service.action_extractor, "extract", new_callable=AsyncMock
            ) as mock_action_extract,
            patch.object(
                service.synthesizer, "synthesize", new_callable=AsyncMock
            ) as mock_synthesize,
        ):
...
class TestIntegration:
    ...
    async def test_transcript_service_intelligence_integration(self):
        ...
        ) as mock_intelligence_service_class:
            mock_intelligence_service = MagicMock()
            mock_intelligence_service.extract_intelligence = AsyncMock(
                return_value=mock_intelligence_result
            )
