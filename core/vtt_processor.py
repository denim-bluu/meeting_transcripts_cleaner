...
class VTTProcessor:
    ...
    def parse_vtt(self, content: str) -> List[VTTEntry]:
        ...
        logger.debug(
            ...
            processing_blocks=len([b for b in blocks if b.strip() and 'WEBVTT' not in b])
        )
        ...
        logger.info(
            ...
            total_duration_seconds=max(entry.end_time for entry in entries) if entries else 0,
            skipped_blocks=skipped_blocks,
            invalid_timestamp_blocks=invalid_timestamp_blocks,
            missing_speaker_blocks=missing_speaker_blocks,
            success_rate=f"{len(entries)/(len(blocks)-skipped_blocks)*100:.1f}%" if len(blocks) > skipped_blocks else "0%"
        )
    ...
    def create_chunks(self, entries: List[VTTEntry], target_tokens: int = 500) -> List[VTTChunk]:
        ...
        for entry_idx, entry in enumerate(entries):
            ...
            if current_tokens + entry_tokens > target_tokens and current_chunk_entries:
                # Log chunk creation
                chunk_speakers = list(set(e.speaker for e in current_chunk_entries))
                logger.debug(
                    "Creating chunk",
                    chunk_id=chunk_id,
                    ...
                    duration_seconds=current_chunk_entries[-1].end_time - current_chunk_entries[0].start_time
                )
        ...
        for chunk in chunks:
            chunk_speakers = set(e.speaker for e in chunk.entries)
        ...
        logger.info(
            ...
            avg_switches_per_chunk=round(total_speaker_switches / len(chunks), 1) if chunks else 0,
            chunk_size_distribution={
                ...
                "median_tokens": sorted([chunk.token_count for chunk in chunks])[len(chunks)//2] if chunks else 0
            }
        )
